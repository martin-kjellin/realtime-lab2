...
/* Calibrates the light sensor when the touch sensor is pressed twice. */
...
/* Makes the car follow a line on the floor. */
...
/* Keeps a constant distance to another object. */
TASK(DistanceTask){
  ...
  if (dis >= 0)
    {
      ...
      if (dis <= 40) /* 40 cm since the sensor is mounted at the back. */
        {
